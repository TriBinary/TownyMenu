 ...
 *   the `/townymenu` parent command (alias `/tm`). A command with
 *   `name = "reload"` becomes `/townymenu reload`.
 * * **Main commands** (`isMainCommand = true`) – registered directly on the
 *   server [CommandMap] as standalone top-level commands.
 *
 * The `/townymenu` command provides tab-completion for all registered
 * sub-commands automatically.
 */
object CommandRegistrar {

    private const val COMMANDS_PACKAGE = "net.trilleo.mc.plugins.townymenu.commands"
    private const val ROOT_COMMAND = "townymenu"
     ...
     * @property isSubCommand `true` when the command is a sub-command of `/townymenu`
     */
     ...
     * and registers it either as a sub-command of `/townymenu` or as a
