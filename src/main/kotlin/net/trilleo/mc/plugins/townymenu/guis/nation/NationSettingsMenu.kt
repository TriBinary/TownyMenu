...
class NationSettingsMenu(player: Player, back: Menu) : Menu(player, player.tr("nation-details.title"), 5, back) {
    ...
    override fun build() {
        ...
        val grid = layout(10, 11, 12, 13, 14, 15, 16, 19, 20, 21, 22, 23, 24, 25)

        grid.add(
            PermissionNodes.TOWNY_COMMAND_NATION_SET_NAME,
            Icons.icon(
                Material.NAME_TAG,
        ...
        grid.add(
            PermissionNodes.TOWNY_COMMAND_NATION_SET_TAG,
            Icons.icon(
                Material.PAPER,
        ...
        grid.add(
            PermissionNodes.TOWNY_COMMAND_NATION_SET_MAPCOLOR,
            Icons.icon(
        ...
        if (TownyUtil.economy) {
            money(
                grid, PermissionNodes.TOWNY_COMMAND_NATION_SET_TAXES, "taxes", tr("nation-details.taxes"),
                if (nation.isTaxPercentage) "${nation.taxes}%" else TownyUtil.money(nation.taxes)
            ) { nation.taxes }
            if (nation.isTaxPercentage) {
                money(
                    grid,
                    PermissionNodes.TOWNY_COMMAND_NATION_SET_TAXPERCENTCAP,
                    "taxpercentcap",
                    tr("town-details.tax-cap"),
                    TownyUtil.money(nation.maxPercentTaxAmount)
                ) { nation.maxPercentTaxAmount }
            }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_NATION_SET_CONQUEREDTAX,
                "conqueredtax",
                tr("nation-details.conquered-tax"),
                TownyUtil.money(nation.conqueredTax)
            ) { nation.conqueredTax }
            money(
                grid, PermissionNodes.TOWNY_COMMAND_NATION_SET_SPAWNCOST, "spawncost", tr("town-details.spawn-cost"),
                TownyUtil.money(nation.spawnCost)
            ) { nation.spawnCost }
        }
