...
class TownSettingsMenu(player: Player, back: Menu) : Menu(player, player.tr("town-details.title"), 6, back) {
    ...
    override fun build() {
        ...
        grid.add(
            PermissionNodes.TOWNY_COMMAND_TOWN_SET_TAG,
            Icons.icon(
                Material.PAPER,
                tr("town-details.tag"),
        ...
        if (TownyUtil.economy) {
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_TAXES,
                Material.GOLD_NUGGET,
                "taxes",
                tr("town-details.taxes"),
                if (town.isTaxPercentage) "${town.taxes}%" else TownyUtil.money(town.taxes)
            ) { town.taxes }
            if (town.isTaxPercentage) {
                money(
                    grid,
                    PermissionNodes.TOWNY_COMMAND_TOWN_SET_TAXPERCENTCAP,
                    Material.GOLD_NUGGET,
                    "taxpercentcap",
                    tr("town-details.tax-cap"),
                    TownyUtil.money(town.maxPercentTaxAmount)
                ) { town.maxPercentTaxAmount }
            }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_PLOTTAX,
                Material.IRON_NUGGET,
                "plottax",
                tr("town-details.plot-tax"),
                TownyUtil.money(town.plotTax)
            ) { town.plotTax }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_SHOPTAX,
                Material.IRON_NUGGET,
                "shoptax",
                tr("town-details.shop-tax"),
                TownyUtil.money(town.commercialPlotTax)
            ) { town.commercialPlotTax }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_EMBASSYTAX,
                Material.IRON_NUGGET,
                "embassytax",
                tr("town-details.embassy-tax"),
                TownyUtil.money(town.embassyPlotTax)
            ) { town.embassyPlotTax }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_PLOTPRICE,
                Material.EMERALD,
                "plotprice",
                tr("town-details.plot-price"),
                TownyUtil.money(town.plotPrice)
            ) { town.plotPrice }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_SHOPPRICE,
                Material.EMERALD,
                "shopprice",
                tr("town-details.shop-price"),
                TownyUtil.money(town.commercialPlotPrice)
            ) { town.commercialPlotPrice }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_EMBASSYPRICE,
                Material.EMERALD,
                "embassyprice",
                tr("town-details.embassy-price"),
                TownyUtil.money(town.embassyPlotPrice)
            ) { town.embassyPlotPrice }
            money(
                grid,
                PermissionNodes.TOWNY_COMMAND_TOWN_SET_SPAWNCOST,
                Material.ENDER_PEARL,
                "spawncost",
                tr("town-details.spawn-cost"),
                TownyUtil.money(town.spawnCost)
            ) { town.spawnCost }

            if (town.isForSale) {
                grid.add(
                    PermissionNodes.TOWNY_COMMAND_TOWN_NOTFORSALE,
                    Icons.icon(
                        Material.RED_BANNER,
                ...
                grid.add(
                    PermissionNodes.TOWNY_COMMAND_TOWN_FORSALE,
                    Icons.icon(
