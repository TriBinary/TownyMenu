...
object RankMenu {
    ...
    fun create(player: Player, target: Resident, nation: Boolean, back: Menu): Menu {
        ...
        val title = "${if (nation) "Nation" else "Town"} Ranks: ${TownyUtil.name(target.name)}"
            ...
            ranks.map { rank ->
                ...
                val icon = {
                    itemStack(if (held) Material.NAME_TAG else Material.PAPER) {
                        ...
                        lore(
                            if (held) "<green>Assigned" else "<gray>Not assigned",
                            if (allowed) "<yellow>Click to ${if (held) "remove" else "assign"}" else "<red>You can't assign this rank.",
                        )
                        glow(held)
                    }
                }
