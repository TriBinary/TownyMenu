...
class TownClaimsMenu(player: Player, back: Menu) : Menu(player, player.tr("claims.title"), 5, back) {
    ...
    override fun build() {
        ...
        button(4, Icons.icon(Material.GRASS_BLOCK, tr("claims.info"), null, *buildList {
            ...
            add(tr("claims.available", "available" to if (town.hasUnlimitedClaims()) tr("claims.unlimited") else town.availableTownBlocks()))
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWN_CLAIM_TOWN_MULTIPLE,
            ...
            prompt(tr("claims.claim-area-title"), tr("claims.radius"), tr("claims.radius-hint"), initial = "1", maxLength = 3) { radius ->
        ...
        if (TownyUtil.economy && TownySettings.getMaxPurchasedBlocks(town) > 0) {
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWN_BUY_BONUS,
                Icons.icon(Material.EMERALD_BLOCK, tr("claims.buy-bonus"), tr("claims.buy-bonus-description"),
                    tr("claims.price-each", "price" to TownyUtil.money(town.bonusBlockCost)))) {
                prompt(tr("claims.buy-bonus-title"), tr("claims.bonus-amount"), initial = "1", maxLength = 4) { amount ->
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_RESIDENT_TOGGLE_TOWNCLAIM,
            Icons.toggle(player, Material.LEATHER_BOOTS, tr("claims.auto-claim"), autoClaim, tr("claims.auto-claim-description"))) {
