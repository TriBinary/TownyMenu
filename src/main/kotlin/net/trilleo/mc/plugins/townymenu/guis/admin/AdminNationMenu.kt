...
class AdminNationMenu(player: Player, private val nation: Nation, back: Menu) :
    ...
    override fun build() {
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_TOGGLE,
            Icons.icon(Material.LEVER, tr("admin-nation.toggles"), tr("admin-nation.toggles-description"))) {
            toggles().open()
        }
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_RENAME,
            Icons.icon(Material.NAME_TAG, tr("admin.rename"), null, tr("common.current", "value" to TownyUtil.name(nation.name)))) {
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_SET,
            Icons.icon(Material.PLAYER_HEAD, tr("nation-details.leader"), tr("admin-nation.leader-description"),
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_ADD,
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_KICK,
            Icons.icon(Material.IRON_BOOTS, tr("admin-nation.kick"), tr("admin-nation.kick-description"),
        ...
        if (TownyUtil.economy) {
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_DEPOSIT,
                Icons.icon(Material.EMERALD, tr("bank.deposit"), tr("admin.deposit-description"), tr("bank.balance", "balance" to TownyUtil.balance(nation)))) {
            ...
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_WITHDRAW,
                Icons.icon(Material.REDSTONE, tr("bank.withdraw"), tr("admin.withdraw-description"), tr("bank.balance", "balance" to TownyUtil.balance(nation)))) {
            ...
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_BANKHISTORY,
                Icons.icon(Material.WRITTEN_BOOK, tr("bank.history"), tr("bank.history-description"))) {
                runAndClose("$command bankhistory")
            }
        }

        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_DELETE,
    ...
    private fun toggles(): Menu {
        fun toggle(material: Material, key: String, name: String, description: String, read: (Nation) -> Boolean) =
            Toggle(material, name, description, PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_NATION_TOGGLE, "$command toggle $key") { read(nation) }

        return ToggleMenu(player, tr("admin-nation.toggles-title", "nation" to TownyUtil.name(nation.name)), this, listOf(
            toggle(Material.WHITE_BANNER, "peaceful", tr("toggle.peaceful"), tr("toggle.nation-peaceful-description")) { it.isNeutral },
            toggle(Material.OAK_DOOR, "open", tr("toggle.open"), tr("toggle.nation-open-description")) { it.isOpen },
            toggle(Material.ENDER_EYE, "public", tr("toggle.public"), tr("toggle.nation-public-description")) { it.isPublic },
            toggle(Material.GOLD_NUGGET, "taxpercent", tr("toggle.tax-percent"), tr("toggle.nation-tax-percent-description")) { it.isTaxPercentage },
        ))
    }
}
