...
class TownMenu(player: Player, back: Menu?) : Menu(player, player.tr("town.title"), 6, back) {
    ...
    override fun build() {
        ...
        button(4, Icons.town(player, town, *buildList {
            ...
            add(
                tr(
                    "town.pvp-mobs",
                    "pvp" to TownyUtil.onOff(player, town.isPVP),
                    "mobs" to TownyUtil.onOff(player, town.hasMobs())
                )
            )
            add(
                tr(
                    "town.fire-explosions",
                    "fire" to TownyUtil.onOff(player, town.isFire),
        ...
        grid.add(
            Icons.icon(
                Material.BEACON, tr("main.nation"),
                if (nation != null) tr(
                    "town.nation-open",
    ...
    fun permissions(): Menu =
        PermissionMenu(
            player, tr("town.permissions-title"), this, "towny:town set perm",
            PermissionNodes.TOWNY_COMMAND_TOWN_SET_PERM, false
        ) { TownyAPI.getInstance().getResident(player)?.townOrNull?.permissions }
    ...
    fun toggles(): Menu {
        ...
        return ToggleMenu(
            player, tr("town.settings-title"), this, listOf(
                toggle(
                    Material.IRON_SWORD,
                    "pvp",
                    tr("toggle.pvp"),
                    tr("toggle.town-pvp-description"),
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_PVP
                ) { it.isPVP },
                toggle(
                    ...
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_MOBS
                ) { it.hasMobs() },
                toggle(
                    ...
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_FIRE
                ) { it.isFire },
                toggle(
                    Material.TNT,
                    "explosion",
                    ...
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_EXPLOSION
                ) { it.isExplosion },
                toggle(
                    Material.OAK_DOOR,
                    "open",
                    tr("toggle.open"),
                    tr("toggle.town-open-description"),
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_OPEN
                ) { it.isOpen },
                toggle(
                    Material.ENDER_EYE,
                    "public",
                    ...
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_PUBLIC
                ) { it.isPublic },
                toggle(
                    ...
                    PermissionNodes.TOWNY_COMMAND_TOWN_TOGGLE_NEUTRAL
                ) { it.isNeutral },
                toggle(
