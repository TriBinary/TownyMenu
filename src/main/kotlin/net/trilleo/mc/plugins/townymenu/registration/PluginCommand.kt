 ...
 * `net.trilleo.mc.plugins.townymenu.commands` package (or any subpackage) to
 * have it automatically discovered and registered at startup.
 *
 * By default every command is registered as a **sub-command** of `/townymenu`
 * (alias `/tm`). For example, a command with `name = "reload"` becomes
 * `/townymenu reload`. Set [isMainCommand] to `true` to register the command
 ...
 *
 * Example (sub-command – registered as `/townymenu start`):
 * ```kotlin
 ...
 *     usage = "/townymenu start",
 *     permission = "townymenu.start"
 * ) {
