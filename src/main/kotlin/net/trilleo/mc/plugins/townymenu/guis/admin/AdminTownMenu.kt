...
class AdminTownMenu(player: Player, private val town: Town, back: Menu) :
    ...
    override fun build() {
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_SPAWN,
            Icons.icon(Material.ENDER_PEARL, tr("admin-town.spawn"), tr("admin-town.spawn-description"))) {
            runAndClose("$command spawn")
        }
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_TOGGLE,
            Icons.icon(Material.LEVER, tr("town.settings"), tr("admin-town.toggles-description"))) {
            toggles().open()
        }
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_RENAME,
            Icons.icon(Material.NAME_TAG, tr("admin.rename"), null, tr("common.current", "value" to TownyUtil.name(town.name)))) {
            prompt(tr("admin.rename"), tr("common.new-name"), initial = town.name) { name ->
                run("$command rename ${TownyUtil.nameArgument(name)}", { town.name })
            }
        }
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_SET_MAYOR,
            Icons.icon(Material.GOLDEN_HELMET, tr("admin-town.mayor"), tr("admin-town.mayor-description"),
            ...
            residentList(tr("admin-town.mayor-title"), { !it.isMayor }) { resident ->
                run("towny:townyadmin set mayor ${town.name} ${resident.name}", { town.mayor }, returnTo = this@AdminTownMenu)
            }
        }
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_KICK,
            Icons.icon(Material.IRON_BOOTS, tr("admin-town.kick"), tr("admin-town.kick-description"),
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_GIVEBONUS,
            Icons.icon(Material.GRASS_BLOCK, tr("admin-town.give-bonus"), tr("admin-town.give-bonus-description"),
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_GIVEBOUGHTBLOCKS,
            Icons.icon(Material.MOSS_BLOCK, tr("admin-town.give-bought"), tr("admin-town.give-bought-description"),
        ...
        if (TownyUtil.economy) {
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_DEPOSIT,
                Icons.icon(Material.EMERALD, tr("bank.deposit"), tr("admin.deposit-description"), tr("bank.balance", "balance" to TownyUtil.balance(town)))) {
            ...
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_WITHDRAW,
                Icons.icon(Material.REDSTONE, tr("bank.withdraw"), tr("admin.withdraw-description"), tr("bank.balance", "balance" to TownyUtil.balance(town)))) {
            ...
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_BANKHISTORY,
        ...
        if (nation != null) {
            ...
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_LEAVENATION,
        ...
        if (town.isRuined) {
            grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_UNRUIN,
        ...
        grid.add(PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_DELETE,
    ...
    private fun toggles(): Menu {
        fun toggle(material: Material, key: String, name: String, description: String, read: (Town) -> Boolean) =
            Toggle(material, name, description, PermissionNodes.TOWNY_COMMAND_TOWNYADMIN_TOWN_TOGGLE, "$command toggle $key") { read(town) }

        return ToggleMenu(player, tr("admin-town.toggles-title", "town" to TownyUtil.name(town.name)), this, listOf(
            toggle(Material.IRON_SWORD, "pvp", tr("toggle.pvp"), tr("toggle.town-pvp-description")) { it.isPVP },
            toggle(Material.ZOMBIE_HEAD, "mobs", tr("toggle.mobs"), tr("toggle.town-mobs-description")) { it.hasMobs() },
            toggle(Material.FLINT_AND_STEEL, "fire", tr("toggle.fire"), tr("toggle.town-fire-description")) { it.isFire },
            toggle(Material.TNT, "explosion", tr("toggle.explosions"), tr("toggle.town-explosions-description")) { it.isExplosion },
            toggle(Material.OAK_DOOR, "open", tr("toggle.open"), tr("toggle.town-open-description")) { it.isOpen },
            toggle(Material.ENDER_EYE, "public", tr("toggle.public"), tr("toggle.town-public-description")) { it.isPublic },
            toggle(Material.WHITE_BANNER, "neutral", tr("toggle.peaceful"), tr("toggle.town-peaceful-description")) { it.isNeutral },
            toggle(Material.GOLD_NUGGET, "taxpercent", tr("toggle.tax-percent"), tr("toggle.tax-percent-description")) { it.isTaxPercentage },
            toggle(Material.DIAMOND_SWORD, "forcepvp", tr("admin-town.force-pvp"), tr("admin-town.force-pvp-description")) { it.isAdminEnabledPVP },
            toggle(Material.WOODEN_SWORD, "forcedisablepvp", tr("admin-town.force-no-pvp"), tr("admin-town.force-no-pvp-description")) { it.isAdminDisabledPVP },
            toggle(Material.CREEPER_HEAD, "forcemobs", tr("admin-town.force-mobs"), tr("admin-town.force-mobs-description")) { it.isAdminEnabledMobs },
            toggle(Material.MAP, "unlimitedclaims", tr("admin-town.unlimited-claims"), tr("admin-town.unlimited-claims-description")) { it.hasUnlimitedClaims() },
            toggle(Material.GOLD_INGOT, "upkeep", tr("admin-town.upkeep"), tr("admin-town.upkeep-description")) { it.hasUpkeep() },
            toggle(Material.RED_BANNER, "allowedtowar", tr("admin-town.war"), tr("admin-town.war-description")) { it.isAllowedToWar },
            toggle(Material.SPYGLASS, "visibleontoplists", tr("admin-town.top-lists"), tr("admin-town.top-lists-description")) { it.isVisibleOnTopLists },
        ))
    }
}
