...
import net.trilleo.mc.plugins.townymenu.guis.nation.NationRelationsMenu
import net.trilleo.mc.plugins.townymenu.guis.nation.NationSettingsMenu
import net.trilleo.mc.plugins.townymenu.guis.nation.NationTownsMenu
...
import net.trilleo.mc.plugins.townymenu.guis.town.OutlawsMenu
import net.trilleo.mc.plugins.townymenu.guis.town.TownClaimsMenu
import net.trilleo.mc.plugins.townymenu.guis.town.TownListMenu
import net.trilleo.mc.plugins.townymenu.guis.town.TownMembersMenu
import net.trilleo.mc.plugins.townymenu.guis.town.TownMenu
import net.trilleo.mc.plugins.townymenu.guis.town.TownSettingsMenu
import net.trilleo.mc.plugins.townymenu.guis.town.TownTrustMenu
 ...
 */
class Link(val requirement: String?, private val condition: (Player) -> Boolean, private val factory: (Player, Menu) -> Menu?) {
...
object Tutorial {
    ...
    private fun withTown(open: (Player, Town, Menu) -> Menu) =
        Link("tutorial.requires-town", { town(it) != null }) { player, back -> town(player)?.let { open(player, it, back) } }

    private fun withNation(open: (Player, Nation, Menu) -> Menu) =
        Link("tutorial.requires-nation", { nation(it) != null }) { player, back -> nation(player)?.let { open(player, it, back) } }
    ...
    val GETTING_STARTED = Chapter(
        ...
        listOf(
            ...
            Lesson("start/menus", Material.KNOWLEDGE_BOOK, "tutorial.start.menus", "tutorial.start.menus-body",
                always { player, _ -> MainMenu(player) }) { player ->
                listOf(player.tr("tutorial.fact.shortcut", "value" to TownyUtil.onOff(player, Main.instance.pluginConfig.sneakSwapHandShortcut)))
            },
            Lesson("start/buttons", Material.GRAY_DYE, "tutorial.start.buttons", "tutorial.start.buttons-body"),
            Lesson("start/wilderness", Material.OAK_SAPLING, "tutorial.start.wilderness", "tutorial.start.wilderness-body") { player ->
                val world = TownyAPI.getInstance().getTownyWorld(player.world)
                if (world == null || !world.isUsingTowny) {
                    listOf(player.tr("tutorial.fact.world-no-towny"))
                } else {
                    listOf(
                        player.tr("tutorial.fact.wild-build", "value" to TownyUtil.yesNo(player, world.getUnclaimedZonePerm(TownyPermission.ActionType.BUILD))),
                        player.tr("tutorial.fact.wild-destroy", "value" to TownyUtil.yesNo(player, world.getUnclaimedZonePerm(TownyPermission.ActionType.DESTROY))),
                    )
                }
            },
            Lesson("start/map", Material.FILLED_MAP, "tutorial.start.map", "tutorial.start.map-body",
                always { player, back -> MapMenu(player, back) }),
            Lesson("start/invites", Material.PAPER, "tutorial.start.invites", "tutorial.start.invites-body",
    ...
    val FINDING_A_TOWN = Chapter(
        ...
        listOf(
            ...
            Lesson("join/browse", Material.OAK_DOOR, "tutorial.join.browse", "tutorial.join.browse-body",
                always { player, back -> TownListMenu(player, back) }),
            Lesson("join/invites", Material.PAPER, "tutorial.join.invites", "tutorial.join.invites-body",
                always { player, back -> InvitesMenu(player, back) }),
            Lesson("join/found", Material.GOLDEN_SHOVEL, "tutorial.join.found", "tutorial.join.found-body",
                Link("tutorial.requires-no-town", { town(it) == null }) { player, back -> NoTownMenu(player, back) }) { player ->
                buildList {
            ...
            Lesson("join/leave", Material.IRON_DOOR, "tutorial.join.leave", "tutorial.join.leave-body",
    ...
    val TOWN = Chapter(
        ...
        withTown { player, _, back -> TownMenu(player, back) },
        listOf(
            Lesson("town/ranks", Material.GOLDEN_HELMET, "tutorial.town.ranks", "tutorial.town.ranks-body",
                withTown { player, town, back -> TownMembersMenu(player, town, back) }),
            Lesson("town/grow", Material.PLAYER_HEAD, "tutorial.town.grow", "tutorial.town.grow-body",
            ...
            Lesson("town/spawn", Material.ENDER_PEARL, "tutorial.town.spawn", "tutorial.town.spawn-body",
            ...
            Lesson("town/settings", Material.LEVER, "tutorial.town.settings", "tutorial.town.settings-body",
            ...
            Lesson("town/details", Material.WRITABLE_BOOK, "tutorial.town.details", "tutorial.town.details-body",
                withTown { player, _, back -> TownSettingsMenu(player, back) }),
            Lesson("town/sell", Material.EMERALD, "tutorial.town.sell", "tutorial.town.sell-body",
                withTown { player, _, back -> TownSettingsMenu(player, back) }, economy),
            Lesson("town/ruins", Material.CRACKED_STONE_BRICKS, "tutorial.town.ruins", "tutorial.town.ruins-body",
                withTown { player, _, back -> TownMenu(player, back) }) { player ->
                buildList {
                    add(player.tr("tutorial.fact.ruins", "value" to TownyUtil.onOff(player, TownySettings.getTownRuinsEnabled())))
                    if (TownySettings.getTownRuinsEnabled()) {
                        add(player.tr("tutorial.fact.ruins-hours", "hours" to TownySettings.getTownRuinsMaxDurationHours()))
                        add(player.tr("tutorial.fact.ruins-reclaim", "value" to TownyUtil.yesNo(player, TownySettings.getTownRuinsReclaimEnabled())))
                    }
                }
            },
        ),
    ...
    val CLAIMS = Chapter(
        ...
        listOf(
            Lesson("claims/chunks", Material.GRASS_BLOCK, "tutorial.claims.chunks", "tutorial.claims.chunks-body",
                ...
                buildList {
                    ...
                    town(player)?.let { add(player.tr("icon.town.claims", "claims" to it.numTownBlocks, "max" to it.maxTownBlocksAsAString)) }
                }
            },
            Lesson("claims/claim", Material.WOODEN_SHOVEL, "tutorial.claims.claim", "tutorial.claims.claim-body",
                withTown { player, _, back -> TownClaimsMenu(player, back) }) { player ->
                buildList {
            ...
            Lesson("claims/auto", Material.LEATHER_BOOTS, "tutorial.claims.auto", "tutorial.claims.auto-body",
                withTown { player, _, back -> TownClaimsMenu(player, back) }),
            Lesson("claims/unclaim", Material.COARSE_DIRT, "tutorial.claims.unclaim", "tutorial.claims.unclaim-body",
                withTown { player, _, back -> TownClaimsMenu(player, back) }) { player ->
                player.money(TownySettings.getClaimRefundPrice(), "tutorial.fact.claim-refund")
            },
            Lesson("claims/outposts", Material.COMPASS, "tutorial.claims.outposts", "tutorial.claims.outposts-body",
                withTown { player, _, back -> TownClaimsMenu(player, back) }) { player ->
                if (!TownySettings.isAllowingOutposts()) {
                    ...
                    buildList {
                        ...
                        town(player)?.let { add(player.tr("claims.outposts", "outposts" to it.maxOutpostSpawn, "limit" to TownySettings.getMaxOutposts(it))) }
                    }
                }
            },
            Lesson("claims/bonus", Material.GOLD_BLOCK, "tutorial.claims.bonus", "tutorial.claims.bonus-body",
                withTown { player, _, back -> TownClaimsMenu(player, back) }, economy) { player ->
    ...
    val PLOTS = Chapter(
        ...
        listOf(
            Lesson("plots/what", Material.GRASS_BLOCK, "tutorial.plots.what", "tutorial.plots.what-body",
                always { player, back -> PlotMenu(player, back) }),
            Lesson("plots/buy", Material.EMERALD, "tutorial.plots.buy", "tutorial.plots.buy-body",
            ...
            Lesson("plots/sell", Material.GREEN_BANNER, "tutorial.plots.sell", "tutorial.plots.sell-body",
                always { player, back -> PlotMenu(player, back) }),
            Lesson("plots/types", Material.OAK_SIGN, "tutorial.plots.types", "tutorial.plots.types-body",
                always { player, back -> PlotMenu(player, back) }),
            Lesson("plots/settings", Material.LEVER, "tutorial.plots.settings", "tutorial.plots.settings-body",
                always { player, back -> PlotMenu(player, back) }),
            Lesson("plots/trust", Material.TRIPWIRE_HOOK, "tutorial.plots.trust", "tutorial.plots.trust-body",
                always { player, back -> PlotMenu(player, back) }),
            Lesson("plots/groups", Material.CHEST, "tutorial.plots.groups", "tutorial.plots.groups-body",
    ...
    val PROTECTION = Chapter(
        ...
        listOf(
            Lesson("protection/actions", Material.IRON_PICKAXE, "tutorial.protection.actions", "tutorial.protection.actions-body"),
            Lesson("protection/groups", Material.PLAYER_HEAD, "tutorial.protection.groups", "tutorial.protection.groups-body"),
            Lesson("protection/town", Material.IRON_DOOR, "tutorial.protection.town", "tutorial.protection.town-body",
                withTown { player, _, back -> TownMenu(player, back).permissions() }),
            Lesson("protection/personal", Material.OAK_DOOR, "tutorial.protection.personal", "tutorial.protection.personal-body",
                always { player, back -> ResidentMenu(player, back).permissions() }),
            Lesson("protection/friends", Material.POPPY, "tutorial.protection.friends", "tutorial.protection.friends-body",
                always { player, back -> FriendsMenu(player, back) }),
            Lesson("protection/trust", Material.TRIPWIRE_HOOK, "tutorial.protection.trust", "tutorial.protection.trust-body",
                withTown { player, town, back -> TownTrustMenu(player, town, back) }),
            Lesson("protection/flags", Material.FLINT_AND_STEEL, "tutorial.protection.flags", "tutorial.protection.flags-body",
                always { player, back -> if (town(player) != null) TownMenu(player, back).toggles() else ResidentMenu(player, back).toggles() }),
            Lesson("protection/outlaws", Material.IRON_BARS, "tutorial.protection.outlaws", "tutorial.protection.outlaws-body",
                withTown { player, town, back -> OutlawsMenu(player, town, back) }),
            Lesson("protection/jail", Material.IRON_CHAIN, "tutorial.protection.jail", "tutorial.protection.jail-body",
                always { player, back -> ResidentMenu(player, back) }) { player ->
                buildList {
                    TownySettings.getJailedMaxHours().takeIf { it > 0 }?.let { add(player.tr("tutorial.fact.jail-hours", "hours" to it)) }
                    resident(player)?.takeIf { it.isJailed }?.let { add(player.tr("tutorial.fact.jailed", "hours" to it.jailHours)) }
                }
            },
        ),
    )
    ...
    val NATIONS = Chapter(
        ...
        listOf(
            Lesson("nations/what", Material.BEACON, "tutorial.nations.what", "tutorial.nations.what-body",
                always { player, back -> NationListMenu(player, back) }),
            Lesson("nations/join", Material.OAK_DOOR, "tutorial.nations.join", "tutorial.nations.join-body",
            ...
            Lesson("nations/found", Material.GOLDEN_HELMET, "tutorial.nations.found", "tutorial.nations.found-body",
            ...
            Lesson("nations/towns", Material.BELL, "tutorial.nations.towns", "tutorial.nations.towns-body",
                withNation { player, nation, back -> NationTownsMenu(player, nation, back) }),
            Lesson("nations/relations", Material.SHIELD, "tutorial.nations.relations", "tutorial.nations.relations-body",
                withNation { player, nation, back -> NationRelationsMenu(player, nation, back) }),
            Lesson("nations/settings", Material.LEVER, "tutorial.nations.settings", "tutorial.nations.settings-body",
            ...
            Lesson("nations/spawn", Material.ENDER_PEARL, "tutorial.nations.spawn", "tutorial.nations.spawn-body",
                withNation { player, _, back -> NationSettingsMenu(player, back) }) { player ->
                listOf(player.tr("tutorial.fact.nation-spawn", "value" to TownyUtil.onOff(player, TownySettings.isConfigAllowingNationSpawn())))
            },
        ),
    )
    ...
    val ECONOMY = Chapter(
        ...
        listOf(
            Lesson("economy/day", Material.CLOCK, "tutorial.economy.day", "tutorial.economy.day-body") { player ->
                ...
                listOf(
                    player.tr("tutorial.fact.new-day", "time" to "%02d:%02d".format(offset / 3600 % 24, offset / 60 % 60)),
                    player.tr("tutorial.fact.daily-taxes", "value" to TownyUtil.onOff(player, TownySettings.isTaxingDaily())),
                )
            },
            Lesson("economy/bank", Material.GOLD_INGOT, "tutorial.economy.bank", "tutorial.economy.bank-body",
            ...
            Lesson("economy/resident-tax", Material.PAPER, "tutorial.economy.resident-tax", "tutorial.economy.resident-tax-body",
                withTown { player, _, back -> TownSettingsMenu(player, back) }) { player ->
                val resident = resident(player)
                val town = resident?.townOrNull
                if (resident == null || town == null) emptyList() else listOf(
                    player.tr("bank.daily-tax", "tax" to if (town.isTaxPercentage) "${town.taxes}%" else TownyUtil.money(town.taxes)),
                    player.tr("profile.tax-owed", "tax" to TownyUtil.money(resident.getTaxOwing(true))),
                )
            },
            Lesson("economy/plot-tax", Material.GOLD_NUGGET, "tutorial.economy.plot-tax", "tutorial.economy.plot-tax-body",
                withTown { player, _, back -> TownSettingsMenu(player, back) }) { player ->
                town(player)?.let { listOf(player.tr("town.plot-tax", "tax" to TownyUtil.money(it.plotTax))) }.orEmpty()
            },
            Lesson("economy/upkeep", Material.HOPPER, "tutorial.economy.upkeep", "tutorial.economy.upkeep-body") { player ->
                ...
                buildList {
                    ...
                    if (nation != null) addAll(player.money(TownySettings.getNationUpkeepCost(nation), "tutorial.fact.nation-upkeep"))
                }
            },
            Lesson("economy/nation-tax", Material.BEACON, "tutorial.economy.nation-tax", "tutorial.economy.nation-tax-body",
                withNation { player, nation, back -> BankMenu(player, nation, back) }) { player ->
                nation(player)?.let {
                    listOf(player.tr("nation.daily-tax", "tax" to if (it.isTaxPercentage) "${it.taxes}%" else TownyUtil.money(it.taxes)))
                }.orEmpty()
            },
            Lesson("economy/bankruptcy", Material.BARRIER, "tutorial.economy.bankruptcy", "tutorial.economy.bankruptcy-body") { player ->
                buildList {
                    add(player.tr("tutorial.fact.bankruptcy", "value" to TownyUtil.onOff(player, TownySettings.isTownBankruptcyEnabled())))
                    town(player)?.takeIf { it.isBankrupt }?.let { add(player.tr("tutorial.fact.debt", "debt" to TownyUtil.money(it.debtBalance))) }
                }
            },
        ),
        economy,
    ...
    val PROFILE = Chapter(
        ...
        listOf(
            Lesson("profile/menu", Material.PLAYER_HEAD, "tutorial.profile.menu", "tutorial.profile.menu-body",
                always { player, back -> ResidentMenu(player, back) }),
            Lesson("profile/others", Material.SPYGLASS, "tutorial.profile.others", "tutorial.profile.others-body",
                always { player, back -> town(player)?.let { TownMembersMenu(player, it, back) } ?: TownListMenu(player, back) }),
            Lesson("profile/display", Material.GLOWSTONE_DUST, "tutorial.profile.display", "tutorial.profile.display-body",
                always { player, back -> ResidentMenu(player, back).toggles() }),
            Lesson("profile/spawn", Material.RED_BED, "tutorial.profile.spawn", "tutorial.profile.spawn-body",
                always { player, back -> ResidentMenu(player, back) }),
        ),
    )
