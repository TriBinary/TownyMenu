...
class LangFilesTest {
    ...
    /** Keys built at runtime (`"plot-type.$type"`), which the source scan cannot see. */
    private val dynamicPrefixes = listOf("plot-type.", "command.")
    ...
    /** Dotted string literals whose first segment is a section of en_US.yml, so permission nodes don't count. */
    private fun usedKeys(): Set<String> {
        ...
        return File("src/main/kotlin").walk()
            ...
            .flatMap { file -> keyLiteral.findAll(file.readText()).map { it.groupValues[1] } }
